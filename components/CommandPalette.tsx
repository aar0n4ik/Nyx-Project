...
const APP_URL = "/nyx/index.html";

type Cmd = { id: string; label: string; group: string; hint?: string; keywords: string; run: () => void };
...
export default function CommandPalette() {
  ...
  const cmds = useMemo<Cmd[]>(() => {
    ...
    const nav = (id: string) => () => { close(); goTo(id); };
    const ext = (url: string) => () => { close(); window.open(url, "_blank", "noopener,noreferrer"); };
    ...
    return [
      { id: "bet", label: pick(lang, { en: "Place a bet", ru: "Сделать ставку", es: "Hacer una apuesta", pt: "Fazer uma aposta", fr: "Placer un pari", de: "Wette platzieren", zh: "下注" }), group: gActions, hint: "Blink", keywords: "bet stake wager blink solana", run: () => { setOpen(false); window.dispatchEvent(new CustomEvent("nyx-open-blink", { detail: { url: BLINK_ACTION } })); } },
      { id: "app", label: pick(lang, { en: "Open the app", ru: "Открыть приложение", es: "Abrir la app", pt: "Abrir o app", fr: "Ouvrir l'app", de: "App öffnen", zh: "打开应用" }), group: gActions, keywords: "app dashboard launch pwa", run: () => { setOpen(false); window.location.href = APP_URL; } },
      ...
      { id: "nav-live", label: pick(lang, { en: "Live constellation", ru: "Живое созвездие", es: "Constelación en vivo", pt: "Constelação ao vivo", fr: "Constellation en direct", de: "Live-Konstellation", zh: "实时星座" }), group: gNav, keywords: "live constellation activity globe", run: nav("live") },
      ...
      { id: "nav-proof", label: pick(lang, { en: "Verify on-chain", ru: "Проверить ончейн", es: "Verificar on-chain", pt: "Verificar on-chain", fr: "Vérifier on-chain", de: "On-chain überprüfen", zh: "链上验证" }), group: gNav, keywords: "proof verify chain inference", run: nav("verify") },
      ...
      { id: "nav-founder", label: pick(lang, { en: "About the founder", ru: "О создателе", es: "Sobre el fundador", pt: "Sobre o fundador", fr: "À propos du fondateur", de: "Über den Gründer", zh: "关于创始人" }), group: gNav, keywords: "founder about", run: nav("founder") },
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return cmds;
    return cmds.filter(
      (c) => c.label.toLowerCase().indexOf(q) >= 0 || c.keywords.indexOf(q) >= 0 || c.group.toLowerCase().indexOf(q) >= 0
    );
  }, [query, cmds]);
  ...
  const onPanelKey = (e: React.KeyboardEvent) => {
    ...
    if (e.key === "ArrowDown") { e.preventDefault(); setActive((i) => (filtered.length ? (i + 1) % filtered.length : 0)); return; }
    if (e.key === "ArrowUp") { e.preventDefault(); setActive((i) => (filtered.length ? (i - 1 + filtered.length) % filtered.length : 0)); return; }
    if (e.key === "Enter") { e.preventDefault(); const cmd = filtered[active]; if (cmd) cmd.run(); }
  };
        ...
        >
          ...
          >
            <div className="flex items-center gap-3 border-b border-hairline px-4">
              ...
              />
              <span className="shrink-0 rounded border border-hairline px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-muted">ESC</span>
            </div>

            <div className="nyx-scroll max-h-[52vh] overflow-y-auto py-2">
              {filtered.length === 0 ? (
                ...
                filtered.map((c, i) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => c.run()}
                      className={"flex w-full items-center justify-between px-4 py-2.5 text-left text-sm transition-colors " + (isActive ? "bg-subtle text-ink" : "text-muted")}
                    >
                      <span className="flex items-center gap-3">
                        <span className="w-20 shrink-0 text-[10px] uppercase tracking-wide text-muted">{c.group}</span>
                        <span className={isActive ? "text-ink" : "text-ink/80"}>{c.label}</span>
                      </span>
                      {c.hint ? (
                        <span className="rounded bg-subtle px-1.5 py-0.5 text-[10px] uppercase tracking-wide text-muted">{c.hint}</span>
                      ) : isActive ? (
            ...
            <div className="flex items-center justify-between border-t border-hairline px-4 py-2.5 text-[11px] text-muted">
              <span className="flex items-center gap-3">
                <span className="flex items-center gap-1"><ArrowUp className="h-3 w-3" /><ArrowDown className="h-3 w-3" />{pick(lang, { en: "navigate", ru: "перемещение", es: "navegar", pt: "navegar", fr: "naviguer", de: "navigieren", zh: "导航" })}</span>
                <span className="flex items-center gap-1"><CornerDownLeft className="h-3 w-3" />{pick(lang, { en: "select", ru: "выбрать", es: "seleccionar", pt: "selecionar", fr: "sélectionner", de: "auswählen", zh: "选择" })}</span>
              </span>
              <span className="font-mono uppercase tracking-wide">Nyx</span>
            </div>
          </motion.div>
        </motion.div>
      ) : null}
